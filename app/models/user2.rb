...
class User < ActiveRecord::Base
  ...
  def feed_objects(last_obj_time)
  ...
  def ordered_feed_objs(last_an_time, last_qn_time)

    topics_weightage = self.weightage_topic()
    followed_user_ids = self.users_followed_ids
    followed_topic_ids = self.topics_followed_ids
    feed_questions = Question.find_by_sql ["SELECT qn_list.id AS id,
              ...
              LIMIT 15",  {followed_ids: followed_user_ids, topic_ids: followed_topic_ids, last_feed_qn_time: last_qn_time} ]
    ...
    feed_answers = Answer.find_by_sql ["SELECT an_list.id AS id,
            ...
            ORDER BY max(sort_time_c) DESC
            LIMIT 35",  {followed_ids: followed_user_ids, last_feed_an_time: last_an_time} ]
  ...
  def profile_view(last_obj_sort_time)
    ...
    if last_obj_sort_time == "0"
      prof_objs = cached_objects[0, num_results]
    else
      last_obj_index = cached_objects.find_index{|obj| obj.sorted_time_str == last_obj_sort_time}
      prof_objs = cached_objects[last_obj_index + 1 , num_results]
    end

    preload_qns_ans(prof_objs)
    return prof_objs
